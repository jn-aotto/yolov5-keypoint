...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.my_kpt  = my_kpt
        self.flip_index = [0, 2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15] # 修改关键点个数
        self.flipLR_index = [1,0,3,2,5,4,7,6][:my_kpt] # 修改关键点个数
        ...
        labels,shapes, self.segments = zip(*cache.values()) # 去掉label
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
#--------------------------------------------------------------关键点修改部分                            
                            #assert l.shape[1] == 56, 'labels require 56 columns each' #修改标签个数

                            assert l.shape[1] >= (1 + 4 + self.my_kpt * 2), 'labels require 56 columns each' #修改标签个数
                            ...
                            #kpts = np.zeros((l.shape[0], (1 + 4 + self.my_kpt * 2)))
                            ...
                            assert l.shape[1] == 1 + 4 + self.my_kpt * 2, 'labels require 39 columns each after removing occlusion paramater'                          
                        else:
                        ...
                        l = np.zeros((0, 1 + 4 + self.my_kpt * 2), dtype=np.float32) if kpt_label else np.zeros((0, 5), dtype=np.float32)
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            if random.random() < hyp['fliplr']:
                ...
                if nL:
                    labels[:, 1] = 1 - labels[:, 1]
                    if self.kpt_label and self.my_kpt == 17:
...
def load_mosaic9(self, index):
    ...
    # Augment
    img9, labels9 = random_perspective(self,img9, labels9, segments9,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
...
#--------------------------------修改关键点个数
        ...
        if kpt_label:
            ...
            xy_kpts = np.ones((n * self.my_kpt, 3))
            ...
            xy_kpts[:, :2] = targets[:,5:].reshape(n * self.my_kpt, 2)  #num_kpt is hardcoded to 17
            ...
            xy_kpts = (xy_kpts[:, :2] / xy_kpts[:, 2:3] if perspective else xy_kpts[:, :2]).reshape(n, self.my_kpt * 2)  # perspective rescale or affine                
            ...
            x_kpts = xy_kpts[:, list(range(0,self.my_kpt * 2,2))]
            y_kpts = xy_kpts[:, list(range(1,self.my_kpt * 2,2))]
            ...
            xy_kpts[:, list(range(0, self.my_kpt * 2, 2))] = x_kpts
            xy_kpts[:, list(range(1, self.my_kpt * 2, 2))] = y_kpts
        # filter candidates
