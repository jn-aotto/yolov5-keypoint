...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    if nc is None:
        ...
        nc = prediction.shape[2] - 5  if not kpt_label else prediction.shape[2] - (my_kpt * 3 + 5) # number of classes
...
def non_max_suppression_export(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    if nc is None:
        ...
        nc = prediction.shape[2] - 5  if not kpt_label else prediction.shape[2] - (my_kpt * 3 + 5) # number of classes
    min_wh, max_wh = 2, 4096  # (pixels) minimum and maximum box width and height
    xc = prediction[..., 4] > conf_thres  # candidates 候选
    
    #output = [torch.zeros((0, 57), device=prediction.device)] * prediction.shape[0]
    output = [torch.zeros((0,6 + my_kpt * 3), device=prediction.device)] * prediction.shape[0]
