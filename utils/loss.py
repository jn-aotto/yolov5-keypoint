...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, kpt_label=False,kpt_num=17,nc=1):
...
#--------------------修改部分
        ...
        sigmas = torch.tensor([.25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25, .25][:4], device=device) / (4 /4)# 四个惩罚项
#-----------------------修改部分        
...
#------------------------修改部分            
            ...
            gain = torch.ones(7 + self.nkpt * 2, device=targets.device)  # normalized to gridspace gain # 归一化增益
#-------------------------------
        else:
