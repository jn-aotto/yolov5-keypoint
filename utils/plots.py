...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=3, orig_shape=None):
...
#--------------------修改调整部分 调整成四个点
    skeleton = [[2, 3], [1, 2], [1, 3], [2, 4] ] # 四个点
    pose_limb_color = palette[[9, 7, 0, 0, 16, 16]] #关键点个数加2
    pose_kpt_color = palette[[16, 16,  0,  9,  9, 9]] #关键点个数加2
    ...
    for kid in range(num_kpts):
        ...
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            ...
            try:
                ...
                cv2.putText(im, str(kid), (int(x_coord)+20, int(y_coord)+20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
            except:
                cv2.circle(im, (int(0), int(0)), radius, (int(r), int(g), int(b)), -1)
                cv2.putText(im, str(kid), (int(0)+20, int(0)+20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)                
...
#----------------------- 减去 个数 * 2  13 * 2 = 26    40-26 = 14
            labels = image_targets.shape[1] == 14 if kpt_label else image_targets.shape[1] == 6   # labels if no conf column
#-----------------------修改            
