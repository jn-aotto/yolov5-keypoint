...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=None, ch=(), inplace=True, dw_conv_kpt=False):  # detection layer
        ...
        #self.na = len(anchors[0]) // 2 # number of anchorstorch.div(dim_t, 2, rounding_mode='trunc')
        self.na = torch.div(len(anchors[0]), 2, rounding_mode='trunc') # number of anchorstorch.div(dim_t, 2, rounding_mode='trunc')
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    na = torch.div(len(anchors[0]), 2, rounding_mode='trunc')  if isinstance(anchors, list) else anchors  # number of anchors
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, ConvFocus, CrossConv, BottleneckCSP,
            ...
            #c2 = ch[f] // args[0] ** 2
            c2 = torch.div(ch[f] ,args[0],rounding_mode='trunc') ** 2
        else:
            c2 = ch[f]
